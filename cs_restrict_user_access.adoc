...
summary: Detenga inmediatamente los ataques detectados restringiendo el acceso de los usuarios comprometidos de forma automática o manual, evitando así mayores daños a los sistemas de archivos en múltiples volúmenes de almacenamiento. 
---
= Bloquear el acceso de usuarios para detener ataques
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Detenga inmediatamente los ataques detectados bloqueando el acceso de los usuarios comprometidos para evitar más daños o exfiltración de datos. Workload Security permite tanto el bloqueo automático a través de políticas de respuesta automatizadas como la intervención manual desde páginas de alertas o detalles del usuario, lo que le brinda un control flexible sobre su respuesta de seguridad. Las restricciones de acceso se aplican automáticamente en todos los volúmenes de almacenamiento monitoreados y tienen un límite de tiempo para la restauración automática.
