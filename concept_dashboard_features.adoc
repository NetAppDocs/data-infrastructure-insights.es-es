...
summary: Domina las funciones avanzadas de los paneles de control, como la personalización de widgets, el filtrado de consultas, la configuración de variables, las transformaciones de datos y las opciones de formato para crear visualizaciones potentes y flexibles. 
...
[role="lead"]
Aprovecha todo el potencial de tus paneles de control gracias a sus funciones avanzadas y opciones de personalización. Aprende a configurar consultas de widgets, aplicar transformaciones de datos, usar variables para el filtrado dinámico e implementar formatos sofisticados para crear visualizaciones altamente eficaces y personalizadas que se adapten a tus necesidades de supervisión de la infraestructura.
...
== Agrupación, identificación y agregación
...
Los datos que se muestran en un widget se agrupan (a veces se denominan acumulados) a partir de los puntos de datos subyacentes recopilados durante la adquisición. Por ejemplo, si tiene un widget de gráfico de líneas que muestra IOPS de almacenamiento a lo largo del tiempo, es posible que desee ver una línea separada para cada uno de sus centros de datos, para una comparación rápida. En el campo "Agrupar por", seleccione el tipo de objeto para ver líneas, áreas, barras, columnas, etc. individuales (dependiendo del tipo de widget) para cada objeto.  Puedes optar por agrupar por cualquier atributo disponible en la lista para ese objeto.  Por ejemplo, al visualizar los datos de un volumen interno, es posible que desee agrupar los datos por el nombre de almacenamiento.

Para ver los datos consolidados, elimine cualquier atributo de Agrupar por, que por defecto agrupará por "Todos".

Puedes optar por agregar estos datos de varias maneras:
...
Al visualizar el panel de control, al seleccionar la leyenda de cualquier widget cuyos datos estén agrupados por "Todos", se abre una página de consulta que muestra los resultados de la primera consulta utilizada en el widget.
...
=== Identificación de datos personalizados

Cuando crea o modifica un widget que se basa en sus propios datos personalizados, es posible que sus datos no se representen correctamente en el widget sin un atributo identificativo.  Si DII no puede identificar el objeto que ha seleccionado para su widget, le presentará un enlace de _Configuración avanzada_ en el área "Agrupar por".  Amplíe esta sección para seleccionar el atributo con el que desea identificar sus datos.

image:unmodeled_data_identify_by.png["Identificar mediante menú desplegable para datos desconocidos"]
...
Puede alinear aún más sus gráficos agregando puntos de datos en intervalos de minutos, horas o días antes de que esos datos se agrupen posteriormente por atributo (si se elige). Puede elegir agregar puntos de datos según su _Promedio_, _Máximo_, _Mínimo_, _Suma_ o _Recuento_.
