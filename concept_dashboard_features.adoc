...
NOTE: Al crear un monitor, DII examina los datos de la semana anterior para detectar picos o caídas significativas; estos se consideran anomalías. Durante la primera semana tras la creación del monitor (fase de aprendizaje), es posible que aumente el ruido en las alertas. Para mitigar este ruido, solo los picos o caídas que duren más de 30 minutos se consideran anomalías y generan alertas. En la semana siguiente, a medida que se analizan más datos, el ruido suele reducirse y solo un pico o caída significativa posterior se considerará una anomalía.
