...
summary: Acceda a los datos de seguridad de la carga de trabajo a través de una API REST con autenticación segura basada en token para integrarse con CMDB, sistemas de tickets y otras aplicaciones empresariales. 
---
...
[role="lead"]
Integra Workload Security con tu ecosistema empresarial usando una API de REST protegida por autenticación segura basada en tokens. Recupera datos de actividad forense, gestiona tokens de acceso a la API y desarrolla integraciones personalizadas con CMDBs, sistemas de tickets y otras aplicaciones. La documentación interactiva de Swagger proporciona especificaciones completas de la API y te permite probar los endpoints directamente.
