...
** _Detección de anomalías_ supervisa y avisa de cambios en los patrones de tus datos
** _Attribute_ supervisa los cambios en los atributos de un objeto, como el traslado de un objeto a un nuevo centro de datos
...
Los monitores de atributos avisan automáticamente a tu equipo cuando los activos se trasladan entre centros de datos, un grupo de almacenamiento no está cifrado o se produce cualquier modificación en los atributos. Esto ayuda a mantener las líneas base de configuración y a asegurarte de que nada se pase por alto durante nuevas implementaciones o cambios.

image:attribute_monitor_in_list.png["los monitores de atributos se añaden a los tipos de monitor"]

Solo tienes que elegir el tipo de objeto y seleccionar los atributos cuyos cambios quieras supervisar.
