...
summary: Mejore la detección de ataques integrando Workload Security con ONTAP Autonomous Protection para obtener análisis correlacionados, respuestas automatizadas y capacidades de investigación forense. 
...
[role="lead"]
La función de protección autónoma de ONTAP utiliza el análisis de la carga de trabajo en entornos NAS (NFS y SMB) para detectar y advertir de forma proactiva sobre actividad anormal en los archivos que podría indicar ataques maliciosos o modificaciones de datos no autorizadas.
...
Un banner de alta confianza indica que el ataque ha mostrado un comportamiento de manipulación de archivos junto con actividades de cifrado de archivos. El gráfico de archivos cifrados indica la marca de tiempo en la que la solución ARP detectó la actividad de cifrado de volumen.
