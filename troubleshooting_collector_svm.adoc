...
'''
*Problema:* Tras añadir o actualizar un colector de datos SVM de ONTAP (Workload Security), el tenant muestra una alerta de sobreaprovisionamiento o de incumplimiento de suscripción. La capacidad de almacenamiento sin procesar aumentó aunque no se añadió hardware nuevo.

*Prueba esto:* Es posible que el mismo almacenamiento se cuente dos veces — una vez por un recopilador de clúster de Observability ONTAP y otra vez por un recopilador de SVM de Workload Security en el mismo clúster.

Compara la capacidad de almacenamiento bruta que indican los colectores de Observability ONTAP y los colectores SVM de Workload Security. Para cada colector SVM, comprueba si su clúster principal ya está supervisado por un colector de Observability ONTAP. Si es así, solo la capacidad del clúster debería contar para el uso de la suscripción.

Comprueba si se ha producido alguna migración reciente de SVM entre clústeres. Tras la migración de un SVM, es posible que las métricas de capacidad sigan asociadas al clúster anterior hasta que Workload Security reciba información actualizada del colector.

Asegúrate de que el colector de datos ONTAP SVM correspondiente al SVM migrado se encuentre en estado RUNNING. Si es necesario, reinicia el colector para que Workload Security pueda recopilar las métricas correctas del clúster y de la capacidad tras la migración.

Si tanto la capacidad del clúster como la del SVM aparecen en el total, es probable que la capacidad se esté contabilizando por duplicado. Esto suele ocurrir poco después de añadir, actualizar o reiniciar un colector SVM, o de migrar un SVM entre clústeres.

Si el consumo sigue superando tu suscripción, abre un caso de soporte de NetApp con el uso de la suscripción antes y después de la alerta, y una lista de los colectores de Observability y SVM con su capacidad de almacenamiento bruta reportada.

{vacío}

'''
*Problema*: Uno o varios Data Collectors aparecen como Degraded en lugar de Running, aunque el servicio del collector esté activo.

*Prueba esto*: SWS muestra el estado «Degradado» cuando una o más conexiones FPolicy de un nodo se desconectan. Esto no es una interrupción —el colector sigue funcionando—, pero es posible que se pierdan los eventos de auditoría de esos nodos hasta que se restablezca la conectividad.

ONTAP establece una conexión de canal de control FPolicy desde cada nodo que tenga una LIF de datos operativa para el SVM. Un nodo sin una LIF de datos local no puede conectarse. Consulta:

* link:https://docs.netapp.com/us-en/ontap/nas-audit/node-fpolicy-server-communication-process-concept.html["Proceso de comunicación entre un nodo y un servidor FPolicy de ONTAP externo"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/fpolicy-external-fpolicy-servers-concept.html["Cómo funciona ONTAP FPolicy con servidores FPolicy externos"^]
* link:https://docs.netapp.com/us-en/ontap/nas-audit/requirements-best-practices-fpolicy-concept.html["Requisitos, consideraciones y prácticas recomendadas para la configuración de ONTAP FPolicy"^]


En la interfaz de usuario, fíjate en qué nodo(s) están desconectados y luego comprueba si esos nodos tienen un LIF de datos para el SVM.

* No hay LIF de datos en los nodos indicados: ignora el estado Degraded. ONTAP puede mostrar FPolicy desconectada con el motivo "No local lif present to connect to FPolicy server" cuando no hay ninguna LIF de datos local; esto es lo esperado. Consulta link:https://kb.netapp.com/on-prem/ontap/da/NAS/NAS-KBs/Fpolicy_server_not_connected_due_to_no_local_lif_present_on_one_or_more_nodes["KB: El servidor Fpolicy no está conectado debido a "no local lif present""^]. En una futura versión de SWS se dejará de marcar el colector como Degraded en este caso.
* LIF de datos presente en los nodos de la lista: Investiga por qué FPolicy está desconectado (LIF inactiva / falta data-fpolicy-client, red/cortafuegos o reinicio temporal/conmutación por error).
